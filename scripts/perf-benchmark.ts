...
// Track build IDs for chained operations (build → sign → submit)
...
const endpoints: Endpoint[] = [
  ...
  { name: 'BuildMintTx+Signers+Datum', method: 'POST', url: `${TX}/BuildMintTransaction`, data: {
    senderAddress: ADDR_WITH_FUNDS,
    recipientAddress: ADDR_WITH_FUNDS,
    lovelaceAmount: '2000000',
    mintActionsJson: JSON.stringify([{ assetUnit: ASSET_NAME_HEX, quantity: '1' }]),
    mintingPolicyScript: PLUTUS_SCRIPT,
    changeAddress: ADDR_WITH_FUNDS,
  ...
  { name: 'SetCollateral', method: 'POST', url: `${TX}/SetCollateral`, data: {
...
async function callEndpoint(ep: Endpoint): Promise<{ status: number; durationMs: number; dataSize: number; error?: string; responseData?: Record<string, unknown> }> {
  ...
  try {
    ...
    return {
      ...
      durationMs,
      dataSize: 0,
      error: `${axErr.response?.status ?? 'NETWORK'}: ${axErr.message}`,
    };
  }
}

function computeStats(rounds: CallResult[]): EndpointResult['stats'] {
  // Include all durations (even errors) — server processing time is still relevant for benchmarking
...
(async () => {
  ...
  for (const ep of endpoints) {
    ...
    for (let r = 1; r <= ROUNDS; r++) {
      ...
      const icon = res.error ? 'x' : '.';
      process.stdout.write(icon);

      // Capture build ID from first successful BuildSimpleAdaTransaction for chained signing flow
    ...
    const result: EndpointResult = { name: ep.name, category: ep.category, method: ep.method, rounds, stats };
    results.push(result);

    const errCount = rounds.filter(r => !!r.error).length;
    const errSuffix = errCount > 0 ? `  [${errCount}/${rounds.length} failed: ${rounds.find(r => r.error)?.status}]` : '';
    console.log(`  avg=${stats.avg}ms  cold=${stats.coldStart}ms  warm=${stats.warmAvg}ms${errSuffix}`);
  }

  // --- Chained Signing Flow (Build → CreateSigningRequest → GetSigningRequest → GetBuildDetails) ---
  console.log('─'.repeat(80));
  ...
  if (lastBuildId) {
    ...
    for (const ep of chainedEndpoints) {
      ...
      // Only 1 round for chained ops (they create state)
      const res = await callEndpoint(ep);
      rounds.push({ round: 1, ...res });

      // Capture signing request ID for downstream calls
      ...
      const stats = computeStats(rounds);
      results.push({ name: ep.name, category: ep.category, method: ep.method, rounds, stats });
      console.log(`  avg=${stats.avg}ms`);
    }

    // GetSigningRequest (requires a signing request to exist)
    if (signingRequestId) {
