...
const OUTPUT = getArg('output', path.join(__dirname, 'perf-compare-results.json'));
...
function startServer(config: BackendConfig): ChildProcess {
  ...
  proc.stdout?.on('data', (data: Buffer) => {
    ...
    for (const line of lines) {
      ...
      if (line.includes('[cds]') || line.includes('server listening') || line.includes('error') || line.includes('Error')) {
...
function printComparison(result: CompareResult) {
  ...
  for (const epName of allEndpoints) {
    ...
    const avgStrs = avgs.map(a => a !== null ? `${a.toFixed(1)}ms`.padStart(15) : 'N/A'.padStart(15));
    ...
    console.log(`  ${epName.padEnd(35)}${avgStrs.join('')}${diffStr.padStart(10)}`);
  }

  // Category comparison
  console.log('\n' + '─'.repeat(90));
  console.log(`  ${'Category'.padEnd(35)}${names.map(n => n.padStart(15)).join('')}${'Diff'.padStart(10)}`);
  console.log('─'.repeat(90));

  const categories = [...new Set(backends.flatMap(b => b.report.results.map(r => r.category)))];
  for (const cat of categories) {
    const catAvgs = backends.map(b => {
      const catResults = b.report.results.filter(r => r.category === cat);
      if (catResults.length === 0) return null;
      return Math.round((catResults.reduce((s, r) => s + r.stats.avg, 0) / catResults.length) * 100) / 100;
    });

    const catStrs = catAvgs.map(a => a !== null ? `${a.toFixed(1)}ms`.padStart(15) : 'N/A'.padStart(15));
    ...
    console.log(`  ${cat.padEnd(35)}${catStrs.join('')}${diffStr.padStart(10)}`);
  }
  ...
  for (const b of backends) {
    ...
    console.log(`    ${b.name.padEnd(15)} avg=${s.avgResponseMs.toFixed(1)}ms  success=${s.successfulCalls}/${s.totalCalls}  startup=${(b.startupMs / 1000).toFixed(1)}s`);
  }
  console.log('═'.repeat(90));
}
...
(async () => {
  ...
  for (const config of BACKEND_CONFIGS) {
    const keyName = config.name === 'blockfrost' ? 'BLOCKFROST_API_KEY' : 'KOIOS_API_KEY';
    const keyValue = config.env[keyName];
    if (!keyValue) {
      console.error(`  ERROR: ${keyName} not set in .env — cannot benchmark ${config.name}`);
      process.exit(1);
    }
  }
  ...
  for (const config of BACKEND_CONFIGS) {
    ...
    try {
      ...
      console.error(`  ERROR running benchmark for ${config.name}: ${err.message}`);
      compareResult.backends.push({
        ...
        report: {
          ...
          results: [],
          summary: { totalEndpoints: 0, totalCalls: 0, successfulCalls: 0, failedCalls: 0, avgResponseMs: 0, slowest: { name: '', avgMs: 0 }, fastest: { name: '', avgMs: 0 } },
        },
      });
    } finally {
      // Kill server
